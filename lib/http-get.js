...
var head = function (options, cb, reqId) {
	...
	options.nogzip = true; // the legacy interface throws a 'Parse Error' on Accept-Encoding: gzip
	...
	// the request boilerplate for supporting both client modes
	...
	// patches the legacy interface in order to avoid #1202
	if (url.protocol != 'https:') {
		// patches the host header so HTTP/1.1 servers won't reply with 400
		opt.headers.host = opt.headers.host || opt.host;
		
		var cl = http.createClient(opt.port, opt.host, client);
		
		cl.on('error', function (err) {
			cb(err);
		});
		
		var req = cl.request(opt.method, opt.path, opt.headers);
		processRequest(req);
	} else {
		// use the default https client
		// currently unstable with broken names
