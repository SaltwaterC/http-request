...
var get = function (options, file, cb, reqId) {
	...
	if (options.progress && ! (typeof options.progress == 'function')) {
		cb(new Error('The progress option must be a callback function.'));
		return;
	}
	...
	var processRequest = function (req) {
		req.on('response', function (res) {
			switch (res.statusCode) {
				case 200: // success
					if (options.progress) {
						var totalLen = absInt(res.headers['content-length']);
						var currLen = 0;
					}
					if (options.nogzip || ! gunzipstream) {
					...
					if (file) {
						...
						if (res.headers['content-encoding'] == 'gzip') {
							...
							gunzip.on('data', function (data) {
								if (options.progress && data && data.length) {
									currLen = currLen + data.length;
									options.progress(currLen, totalLen);
								}
								try {
									ws.write(data);
				...
				case 417:
					...
					var fetchBody = true;
					// this is just some nasty patchwork to make it going, till http-get v0.4
					var buf = '';
					if (res.headers['content-encoding'] == 'gzip') {
						var gunzip = gunzipstream.wrap(res, {encoding: null});
						gunzip.on('error', function (err) {
