 ...
 * @param {module:request~stdResult} result The standard result. If you wrap {@link module:request~Request#saveBuffer} or {@link module:request~Request#saveFile} by yourself, you have to use the result passed to innerCallback instead of calling {@link module:request~Request#stdResult} for consistent results. When the request is reissued for a plain resource due to gzip or deflate decoding errors a new {@link module:request~Request} object is created internally in order to keep things simpler for requests that respond with success, but break when decoded. saveBuffer and saveFile use a simple recursive logic in this particular case
 */
