...
/**
 * Packages the Error object with all the relevant information in a single error exit point
 *
...
Request.prototype.error = function(error, callback) {
	tools.debug('error %s %s; aborted', this.options.method, this.options.url, this.aborted);
...
/**
 * The success exit point of the Request class
 *
 * @param {Object} result The result argument, depending on implementation
 * @param {Function (null, result)} callback Completion callback
 */
Request.prototype.success = function(result, callback) {
	tools.debug('success %s %s; aborted: %s', this.options.method, this.options.url, this.aborted);

	if (!this.aborted) {
		// XXX: need to debug the GET method calling twice every success/error case
		// XXX: defining finish breaks a couple of tests
		//this.finish = true;
...
Request.prototype.saveBuffer = function(callback) {
	...
	self.stream.on('end', function() {
		...
		self.success(self.buffer, callback);
	});
...
Request.prototype.saveFile = function(file, callback) {
	...
	ws.on('close', function() {
		...
		if (self.response.headers['last-modified']) {
			...
			self.success(self.file, callback);
		}
	});
...
return {
  ...
  url: request.url, // undefined | String, the final URL if succesful redirect
