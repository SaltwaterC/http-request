			...
			case 307:
				...
				if (self.options.noRedirect) {
					...
					if (!response.headers.location) {
						...
						if (self.options.reqId < self.options.maxRedirects + 1) {
							...
							self.error(tools.formattedError('Redirect loop detected after %d requests.', self.options.reqId - 1), callback);
						}
					}
				}
				break;
 ...
 * @property {Stream | undefined} stream The HTTP response body if {@link module:request~options}.stream is true as [Readable Stream](http://nodejs.org/api/stream.html#stream_class_stream_readable). The actual stream is either the [HTTP client response](http://nodejs.org/api/http.html#http_event_response) or the decoded stream by [zlib.createUnzip](http://nodejs.org/api/zlib.html#zlib_zlib_createunzip_options) if the Content-Encoding response header is either gzip or deflate
 */
