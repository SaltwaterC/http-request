...
http.put({
	...
	headers: {
		// you need to define a type if required by the target application
...
// put a readable stream with known size and type
http.put({
	url: 'http://localhost/put',
...
exports.put = function(options, file, callback) {
	...
	if (options.reqBody instanceof ReadStream) {
		...
		fs.stat(options.reqBody.path, function(err, stats) {
			...
			/*jslint todo:true*/
			// TODO: get the content-type by using a proper mime lib
			// TODO: try mmmagic
			/*jslint todo:false*/
